...
/// catalog with a top-level `models` field.  Return the zx-code–managed model
...
/// zx-code–owned `model_catalog_json`, using the same path ownership rules as
...
pub async fn handle_models() -> Result<Json<Value>, ProxyError> {
    ...
    let active_catalog_path = match crate::codex_config::read_codex_config_text() {
        Ok(config_text) => {
            crate::codex_config::resolve_zx_code_catalog_path(&config_text, &generated_path)
        }
        Err(_) => None,
    };
        ...
        active_catalog_path.as_ref().filter(|path| path.exists())
    {
        ...
        if active_catalog_path.is_none() {
            log::debug!(
                "[models] stale guard: catalog not served (model_catalog_json not set to zx-code catalog)"
            );
        }
        json!({"models": []})
    };
...
fn codex_proxy_error_json(
    ...
    let message = if upstream_status == Some(413) {
        ...
        // 避免「以为是 ZX Code 封装了 nginx / 是本地代理的锅」这种反复出现的误解。
        format!(
            concat!(
                ...
                "provider's server-side limit, not a ZX Code limit. ",
        ...
        let status_fragment = upstream_status
            ...
            .unwrap_or_default();
        format!(
            "ZX Code local proxy failed while handling Codex endpoint {endpoint}. Provider: {provider_name}; model: {request_model}{status_fragment}; cause: {cause}"
        )
    };
...
fn codex_proxy_error_code(error: &ProxyError) -> &'static str {
    match error {
        ProxyError::ForwardFailed(_) => "zx_code_forward_failed",
        ProxyError::Timeout(_) | ProxyError::StreamIdleTimeout(_) => "zx_code_timeout",
        ProxyError::NoAvailableProvider => "zx_code_no_available_provider",
        ProxyError::AllProvidersCircuitOpen => "zx_code_all_providers_circuit_open",
        ProxyError::NoProvidersConfigured => "zx_code_no_providers_configured",
        ProxyError::MaxRetriesExceeded => "zx_code_max_retries_exceeded",
        ProxyError::ProviderUnhealthy(_) => "zx_code_provider_unhealthy",
        ProxyError::ConfigError(_) => "zx_code_config_error",
        ProxyError::TransformError(_) => "zx_code_transform_error",
        ProxyError::InvalidRequest(_) => "zx_code_invalid_request",
        ProxyError::AuthError(_) => "zx_code_auth_error",
        ProxyError::UpstreamError { .. } => "zx_code_upstream_error",
        ProxyError::DatabaseError(_) => "zx_code_database_error",
        ProxyError::Internal(_) => "zx_code_internal_error",
        ProxyError::AlreadyRunning
        | ProxyError::NotRunning
        | ProxyError::BindFailed(_)
        | ProxyError::StopTimeout
        | ProxyError::StopFailed(_) => "zx_code_proxy_error",
    }
}
...
data: {\"type\":\"response.output_item.done\",\"item\":{\"type\":\"message\"}}\n\n";
    ...
    fn codex_proxy_forward_error_includes_context_and_cause() {
        ...
        assert!(message.contains("ZX Code local proxy failed"));
        ...
        assert_eq!(body["error"]["code"], "zx_code_forward_failed");
