...
const ZX_CODE_SQL_EXPORT_HEADER: &str = "-- ZX Code SQLite 导出";
...
impl Database {
    ...
    fn import_sql_string_inner(
        ...
        let sql_content = sql_raw.trim_start_matches('\u{feff}');
        Self::validate_zx_code_sql_export(sql_content)?;
    ...
    fn validate_zx_code_sql_export(sql: &str) -> Result<(), AppError> {
        let trimmed = sql.trim_start();
        if trimmed.starts_with(ZX_CODE_SQL_EXPORT_HEADER) {
            return Ok(());
        }
...
mod tests {
    ...
    fn periodic_maintenance_runs_even_when_auto_backup_disabled() -> Result<(), AppError> {
        ...
        let test_home =
            std::env::temp_dir().join("zx-code-periodic-maintenance-backup-disabled-test");
        let _ = std::fs::remove_dir_all(&test_home);
        std::fs::create_dir_all(&test_home).expect("create test home");
        std::env::set_var("ZX_CODE_TEST_HOME", &test_home);
