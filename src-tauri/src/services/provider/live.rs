...
fn restore_live_settings_for_provider_backfill(
    ...
    // `modelCatalog` is a zx-code–private field whose SSOT is the DB. Live's
...
pub fn read_live_settings(app_type: AppType) -> Result<Value, AppError> {
    match app_type {
        AppType::Codex => {
            ...
            // `modelCatalog` is a zx-code private field that lives only in
            // the DB SSOT plus the `zx-code-model-catalog.json` projection
...
/// into the ZX Code database. Each provider found will be added to the
