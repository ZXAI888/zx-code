...
mod tests {
    ...
    impl TempHome {
        fn new() -> Self {
            ...
            let original_test_home = env::var("ZX_CODE_TEST_HOME").ok();
    ...
    fn with_test_home<T>(test: impl FnOnce(&AppState, &Path) -> T) -> T {
        ...
        std::env::set_var("ZX_CODE_TEST_HOME", temp.path());
    ...
    fn extract_codex_common_config_strips_provider_fields_and_injected_artifacts() {
        ...
        // web_search = "disabled" 是 zx-code 对黑名单网关注入的哨兵；
...
"#;
        ...
        assert!(
            ...
            "should strip the zx-code web_search disabled sentinel, got: {extracted}"
        );
        // 真正可共享的键保留
        assert!(
...
impl ProviderService {
    ...
    /// 已剥离全部供应商专属与 zx-code 注入内容：`model` / `model_provider` /
    ...
    fn extract_codex_common_config(settings: &Value) -> Result<String, AppError> {
        ...
        // - web_search 只剥 zx-code 注入的 "disabled" 哨兵；用户手设的其它值
        //   属于可共享偏好，保留。
        if root
