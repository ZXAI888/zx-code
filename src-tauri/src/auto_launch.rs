...
/// 将 `/path/to/ZX Code.app/Contents/MacOS/ZX Code` 转换为 `/path/to/ZX Code.app`
#[cfg(target_os = "macos")]
...
mod tests {
    ...
    fn test_get_macos_app_bundle_path_valid() {
        let exe_path = std::path::Path::new("/Applications/ZX Code.app/Contents/MacOS/ZX Code");
        let result = get_macos_app_bundle_path(exe_path);
        assert_eq!(
            result,
            Some(std::path::PathBuf::from("/Applications/ZX Code.app"))
        );
    }
    ...
    fn test_get_macos_app_bundle_path_with_spaces() {
        let exe_path =
            std::path::Path::new("/Users/test/My Apps/ZX Code.app/Contents/MacOS/ZX Code");
        let result = get_macos_app_bundle_path(exe_path);
        assert_eq!(
            result,
    ...
    fn test_get_macos_app_bundle_path_not_in_bundle() {
        let exe_path = std::path::Path::new("/usr/local/bin/zx-code");
        let result = get_macos_app_bundle_path(exe_path);
    ...
    fn test_get_macos_app_bundle_path_dev_build() {
        ...
        let exe_path = std::path::Path::new("/Users/dev/project/target/debug/zx-code");
        let result = get_macos_app_bundle_path(exe_path);
        assert_eq!(result, None);
    }
}
