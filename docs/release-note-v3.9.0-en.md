...
ZX Code v3.9.0 is the stable release of the v3.9 beta series (`3.9.0-1`, `3.9.0-2`, `3.9.0-3`).
...
- SQL import is restricted to ZX Code exported backups to reduce the risk of importing unsafe or incompatible SQL dumps.
- Proxy takeover modifies CLI live configs; ZX Code will back up the live config before redirecting it to the local proxy. If you want to revert, disable takeover/stop the proxy and restore from the backup when needed.
...
Visit [Releases](https://github.com/farion1231/zx-code/releases/latest) to download the appropriate version.
...
| File                                    | Description                                        |
| --------------------------------------- | -------------------------------------------------- |
| `ZX-Code-v3.9.0-Windows.msi`          | **Recommended** - MSI installer with auto-update support |
| `ZX-Code-v3.9.0-Windows-Portable.zip` | Portable version, no installation required         |

### macOS

| File                            | Description                                                       |
| ------------------------------- | ----------------------------------------------------------------- |
| `ZX-Code-v3.9.0-macOS.zip`    | **Recommended** - Extract and drag to Applications, Universal Binary |
| `ZX-Code-v3.9.0-macOS.tar.gz` | For Homebrew installation and auto-update                         |
...
| --------------------------------------- | ------------------ | ---------------------------------------------------------------------- |
| Ubuntu / Debian / Linux Mint / Pop!\_OS | `.deb`             | `sudo dpkg -i ZX-Code-*.deb` or `sudo apt install ./ZX-Code-*.deb` |
| Fedora / RHEL / CentOS / Rocky Linux    | `.rpm`             | `sudo rpm -i ZX-Code-*.rpm` or `sudo dnf install ./ZX-Code-*.rpm`  |
| openSUSE                                | `.rpm`             | `sudo zypper install ./ZX-Code-*.rpm`                                |
| Arch Linux / Manjaro                    | `.AppImage`        | Make executable and run directly, or use AUR                           |
| Other distros / Unsure                  | `.AppImage`        | `chmod +x ZX-Code-*.AppImage && ./ZX-Code-*.AppImage`              |
| Sandboxed installation                  | `.flatpak`         | `flatpak install ZX-Code-*.flatpak`                                  |
