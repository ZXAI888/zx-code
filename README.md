...
- **Deep Link Protocol**
  - `zxcode://` protocol registration (all platforms)
...
**ZX Code Storage (v3.8.0 New Architecture)**

- Database (SSOT): `~/.zx-code/zx-code.db` (SQLite, stores providers, MCP, Prompts, Skills)
- Local settings: `~/.zx-code/settings.json` (device-level settings)
- Backups: `~/.zx-code/backups/` (auto-rotate, keep 10)
...
- **SSOT** (Single Source of Truth): All data stored in `~/.zx-code/zx-code.db` (SQLite)
